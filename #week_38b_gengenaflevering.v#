...
Lemma nat_ind3 :
  forall P : nat -> Prop,
    ...
    forall n : nat,
      P n.
Proof.
  intros P H_P0 H_P1 H_P2 H_PSS n.
  assert(consecutive :
           forall x : nat,
             P x /\ P (S x) /\ P (S (S x))).
    intro x.
  ...
  split.
    ...
    split.
      ...
      split.
        exact (IHSx3).
      exact (H_PSS (S x') IHSx1 IHSx2 IHSx3).
...
Proof.
  ...
  assert(consecutive :
           forall x : nat,
             P x /\ P (S x)).
    intro x.
    induction x as [ | | | x' IHx' IHSx' IHSSx'] using nat_ind3.
    split.
      exact (H_P0).
    exact (H_P1).
    split.
    ...
    split.
      exact (H_PSS 0 H_P0 H_P1).
    exact (H_PSS 1 H_P1 (H_PSS 0 H_P0 H_P1)).
    destruct IHSSx' as [IH_PSSx IH_PSSSx].
    destruct IHSx' as [IH_PSx IH_PSSx'].
    split.
      exact (IH_PSSSx).
    exact (H_PSS (S (S x')) IH_PSSx IH_PSSSx).

...
(* Exercise 1: why is there a space in the comment just above
   ...
   When starting a comment with (* it will look for the next *)
   to complete the comment, so writing * ) instead will avoid
   closing the comment. So in the comment above we are talking
   about the multiplication operator inside of parantheses, and
   naturally we would end that sentence with the same token that
   closes a comment.
   Also, it would seem that comments nest.

...
Lemma unfold_product_of_leaves_acc_ic :
  forall (t1 t2 : binary_tree_nat),
...
Lemma about_product_of_leaves_acc :
  forall (t : binary_tree_nat) (a : nat),
  product_of_leaves_acc t a = (product_of_leaves_acc t 1) * a.
Proof.
  intro t.
  induction t as [ t' | t1 IHt1 t2 IHt2 ].
  intro a.
  rewrite ->2 (unfold_product_of_leaves_acc_bc).
  rewrite <- (mult_assoc).
  rewrite -> (mult_1_l).
  reflexivity.

  intro a.
  rewrite ->2 (unfold_product_of_leaves_acc_ic).
  rewrite -> (IHt1 (product_of_leaves_acc t2 1)).
  rewrite -> (IHt1 (product_of_leaves_acc t2 a)).
  rewrite -> (IHt2).
  rewrite -> (mult_assoc).
  reflexivity.
Qed.
...
Lemma product_of_leaves_v1_fits_the_specification_of_product_of_leaves :
  ...
  split.
  intro n.
  rewrite -> unfold_product_of_leaves_acc_bc.
  rewrite -> mult_1_r.
  reflexivity.

  intros t1 t2.
  rewrite -> unfold_product_of_leaves_acc_ic.
  rewrite -> about_product_of_leaves_acc.
  reflexivity.
Qed.
